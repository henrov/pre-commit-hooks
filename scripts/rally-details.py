...
class RallyCache:
    ...
    def __init__(self, options, formatted_id_list):
        self._options = options
        self._formatted_id_list = formatted_id_list

    def _get_rally_workset(self):
        if self._rally is None:
            server, user, password, apikey, workspace, project = rallyWorkset(self._options)
            if apikey:
    ...
    def get_rally_details(self, entity, prefix, formatted_id):
        ...
        if cache_file_path.exists() and (time.time() - os.path.getmtime(cache_file_path)) < 24*3600:
            ...
            if query_result.errors:
                logging.error(f"request could not be successfully serviced for {formatted_id}, error code: {query_result.status_code}")
...
def main() -> None:
    # get the options and list of formatted id's from the command-line arguments
    options = [opt for opt in sys.argv[1:] if opt.startswith('--')]
    formatted_id_list = set([arg.upper() for arg in sys.argv[1:]
                            if not arg.startswith('--')
                                and re.search(r'US[0-9]{2,}|DE[0-9]{2,}|USERSTORY[0-9]{2,}|DEFECT[0-9]{2,}',
                                              arg, flags=re.IGNORECASE)])

    rally_cache = RallyCache(options, formatted_id_list)
